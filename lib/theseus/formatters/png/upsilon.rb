  ...
  module Formatters
    class PNG
      class Upsilon < PNG
        def initialize(maze, options={})
          ...
          maze.height.times do |y|
            ...
            maze.row_length(y).times do |x|
              ...
              solution = solution_grid ? solution_grid[x][y] : 0

              px = @options[:outer_padding] + x * metrics[:inc]

              if (y + x) % 2 == 0
                draw_octogon_cell(canvas, px, py, cell, solution, metrics)
              else
                draw_square_cell(canvas, px, py, cell, solution, metrics)
              end
            end
          end
        ...
        def draw_octogon_cell(canvas, x, y, cell, solution, metrics)
          ...
          fill_poly(canvas, [p1, p2, p3, p4, p5, p6, p7, p8], (solution & cell != 0) ? options[:solution_color] : options[:cell_color])

          any = proc { |x| x | (x << Maze::UNDER_SHIFT) }
          color = proc { |d| (solution & any[d] == 0) ? options[:cell_color] : options[:solution_color] }

          if cell & any[Maze::NE] != 0
            far_p6 = move(p6, metrics[:inc], -metrics[:inc])
            far_p7 = move(p7, metrics[:inc], -metrics[:inc])
            fill_poly(canvas, [p2, far_p7, far_p6, p3], color[Maze::NE])
          ...
          if cell & any[Maze::E] != 0
            ...
            fill_rect(canvas, r1[0], r1[1], r2[0], r2[1], color[Maze::E])
          ...
          if cell & any[Maze::SE] != 0
            ...
            fill_poly(canvas, [p4, far_p1, far_p8, p5], color[Maze::SE])
          ...
          if cell & any[Maze::S] != 0
            ...
            fill_rect(canvas, r1[0], r1[1], r2[0], r2[1], color[Maze::S])
        ...
        def draw_square_cell(canvas, x, y, cell, solution, metrics)
          v = options[:cell_padding] + metrics[:s4]
          p1 = [x + v, y + v]
          p2 = [x + options[:cell_size] - v, y + options[:cell_size] - v]

          fill_rect(canvas, p1[0], p1[1], p2[0], p2[1], (solution & cell != 0) ? options[:solution_color] : options[:cell_color])
